...
import {
  ...
  useGetAllCategoryImages,
  usePostCategoryImage,
} from "../../query/category";
...
const Gallary = () => {
  ...
  const { data: feedImageData, isLoading } = useGetAllCategoryImages(cid);

  const { mutateAsync: deleteFeedImage } = useDeleteCategoryImage();
  ...
  const { mutateAsync: postProductImage } = usePostCategoryImage();
